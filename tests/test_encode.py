...
class TestQminQmaxEncoding(TestCase):
    def test_qmin_qmax(self) -> None:
        ...
        self.assertTrue(all(small * factor < large for small, large in zip(sizes, sizes[1:])))
...
class TestProfiles(TestCase):
    def test_profiles(self) -> None:
        ...
        for codec_name, rate in codecs:
            ...
            self.assertTrue(len(stream.profiles) >= 1)  # check that we're testing something!
            ...
            for profile in stream.profiles:
                ...
                self.assertTrue(stream.profile == profile)
